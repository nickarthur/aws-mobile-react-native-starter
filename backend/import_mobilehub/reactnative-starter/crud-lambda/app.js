...
AWS
  .config
  .update({region: process.env.REGION});

const UNAUTH = 'UNAUTH';

// The DocumentClient class allows us to interact with DynamoDB using normal
// objects. Documentation for the class is available here:
// http://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/DynamoDB/DocumentClient
// .html
...
app.get('/items/pets', (req, res) => {
  console.log('NICK GOT HERE !!!!!!!!!!!!!!!!!! \n');
  // performs a DynamoDB Query operation to extract all records for the
  // cognitoIdentityId in the table
  dynamoDb.query({
    ...
    if (err) {
      ...
      res
        .status(500)
        .json({message: 'Could not load pets'})
        .end();
    } else {
      res
        .json(data.Items)
...
app.post('/items/pets', (req, res) => {
  if (!req.body.name) {
    res
      .status(400)
      .json({message: 'You must specify a pet name'})
      .end();
    return;
  }
  ...
  Object
    .keys(pet)
    .forEach(key => (pet[key] === '' && delete pet[key]));
  ...
  dynamoDb.put({
    ...
    if (err) {
      ...
      res
        .status(500)
        .json({message: 'Could not insert pet'})
        .end();
    } else {
      res.json(pet);
    }
